...
注意力机制已成为深度学习中表征建模与序列学习的核心组件，广泛应用于自然语言处理、计算机视觉与多模态学习等方向。本文围绕注意力机制的关键计算结构与训练稳定性问题展开研究：首先总结自注意力与多头注意力的基础形式，给出其与 Softmax 归一化及缩放因子的数学关系；随后讨论交叉熵目标与正则项对泛化性能的影响，并从优化角度分析 Adam 等自适应优化算法在训练早期的收敛特征；最后在标准分类与序列建模任务上开展对比实验，验证注意力模块配置、学习率策略与正则强度对性能的影响。实验结果表明，合理的注意力头数与温度/缩放设置可在计算开销与精度之间取得更优折中，同时在小样本场景下配合权重衰减与标签平滑能显著提升泛化鲁棒性[1,2,3]。
...
分类与语言建模任务中，交叉熵是最常用的目标函数。标签平滑、权重衰减与 dropout 等正则化方法可缓解过拟合并提升泛化，但其最佳强度依赖数据规模与模型容量[3,4]。在优化层面，SGD 与 Adam 等优化器在收敛性质上存在差异，自适应方法通常在训练早期更稳定[5]。此外，在大规模预训练与微调场景中，dropout 与权重衰减（如 AdamW）等策略也会影响训练动态与最终性能[6,7]。相关工作也讨论了在不同任务设置下归一化与正则化组合的可迁移性[1,8]。
...
缩放因子 $1/\sqrt{d_k}$ 的作用在于控制 logits 的尺度，避免 Softmax 输出过于尖锐或过于平坦，从而影响梯度传播与训练稳定性。

#### 3.2.1.1 数值稳定性与训练现象

当 $d_k$ 较大且缺少缩放时，注意力权重容易出现近似 one-hot 的分布，训练早期可能表现为损失震荡或收敛变慢；引入缩放后分布更平滑，有利于稳定优化过程。
